...
public class BaseEnemy extends Enemy {
    ...
    public BaseEnemy(PlayScreen playScreen, float x, float y) {
        super(playScreen, x, y);

        Array<TextureRegion> keyFrames = new Array<TextureRegion>();

        for (int i = 0; i < 8; i++) {
            keyFrames.add(new TextureRegion(textureAtlas.findRegion("actors"), 16 * i, 48, 16, 16));
        }
    ...
    public void checkMovingDirection() {
        ...
        RayCastCallback rayCastCallback = new RayCastCallback() {
            ...
            public float reportRayFixture(Fixture fixture, Vector2 point, Vector2 normal, float fraction) {
                ...
                if (fraction < 1.0f && fixture.getUserData().getClass() != Character.class) {
    ...
    public void update(float delta) {
        ...
        switch (currentState) {
            case STOMPED:
                setRegion(new TextureRegion(textureAtlas.findRegion("actors"), 16 * 9, 48, 16, 16));
                if (stateTime > 1.0f) {
                    queueDestroy();
                }
                break;
