...
public class GuardShieldEnemy extends Enemy {
    public enum State {
        WALKING,
        SHELLING,
        SPINNING,
    ...
    public GuardShieldEnemy(PlayScreen playScreen, float x, float y) {
        super(playScreen, x, y);

        Array<TextureRegion> keyFrames = new Array<TextureRegion>();
        for (int i = 0; i < 4; i++) {
            keyFrames.add(new TextureRegion(textureAtlas.findRegion("actors"), i * 16, 64, 16, 16));
        }
        walking = new Animation(0.2f, keyFrames);

        keyFrames.clear();

        shelling = new TextureRegion(textureAtlas.findRegion("actors"), 16 * 4, 64, 16, 16);
    ...
    public void update(float delta) {
        ...
        if (playScreen.getCharacterPosition().x + GameManager.V_WIDTH / 2 + 4 > body.getPosition().x )
            active = true;
    ...
    public void interact(Character character) {
        movingRight = character.getX() <= getX();
        spin = true;
    }
    ...
    protected void defBody() {
        ...
        // weakness
        Vector2[] vertices = {
                new Vector2(-8.0f, 8.0f).scl(1 / GameManager.PPM),
                new Vector2(8.0f, 8.0f).scl(1 / GameManager.PPM),
        ...
        RayCastCallback rayCastCallback = new RayCastCallback() {
            ...
            public float reportRayFixture(Fixture fixture, Vector2 point, Vector2 normal, float fraction) {
                ...
                if (fraction < 1.0f) {
                    ...
                    else if (fixture.getUserData().getClass() != Character.class) {
